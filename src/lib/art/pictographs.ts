 ...
 *
 * **글자를 아예 감추지는 않는다.**
 * 처음에는 그림만 보여 줬는데 "무엇을 합치는 건지 헷갈린다" 는 말을 들었다. 맞는 지적이다 —
 * DragonBox 가 숨기는 것은 *표기법*(x, +)이고 배울 대상은 *조작 규칙*이다.
 * 그런데 우리가 배울 대상은 **글자 그 자체**다. 글자를 감추면 연결할 대상이 사라진다.
 *
 * 그래서 그림이 앞장서되 글자는 늘 곁에 붙어 다니고, 익숙해지면 그림이 조용히 빠진다.
 *  - 아직 서툴면 그림 + 작은 글자 (여기서 연결이 생긴다)
 *  - 익숙해지면 글자만. 그림은 말없이 사라진다
 */
export function fadeStage(mastery: number | null | undefined): 0 | 1 | 2 {
	if (mastery == null || mastery < 50) return 1;
	return 2;
}
