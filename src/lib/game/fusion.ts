...
export const FUSION_RECIPES: FusionRecipe[] = [
	...
	{
		/*
		 * **마지막 마을에 조합이 하나는 있어야 한다.**
		 *
		 * 급수 데이터를 한국어문회 공식 자료에 맞춰 고치면서 寺 가 4급II(8지역)로
		 * 올라갔다. 그런데 寺 는 9지역의 **유일한** 조합 부품이었고, 그 바람에
		 * 최종 보스가 "이번 마을에서 배운 글자" 를 물을 수 없게 됐다 —
		 * 예전에 지적받아 고쳤던 바로 그 문제가 되살아난 것이다.
		 *
		 * 系(이어맬 계)는 이제 9지역이고, 係(맬 계)의 실제 구성 부품이며 음도 같다.
		 * 억지로 끼운 것이 아니라 원래 그런 글자다.
		 */
		parts: ['人', '系'],
		result: '係',
		story: '사람과 사람이 실처럼 이어져 매인 사이예요.',
		kind: '형성',
		layout: 'lr',
		soundPart: '系'
	},
