...
**▶ 한자사전 (로그인 없이) — https://hanjasajeon.pages.dev/hanja**
...
배운 글자 두 개를 붙여 **낱말**을 만든다. 849개 낱말이 1028자 중 987자를 덮는다.
...
## 한자사전 — 게임 데이터를 그대로 공개한다

**https://hanjasajeon.pages.dev/hanja** — 가입도 로그인도 필요 없다.

게임은 전부 로그인 뒤에 있어서 검색엔진이 볼 수 있는 것이 하나도 없었다.
그런데 이 프로젝트가 쌓아 온 데이터(1,028자의 훈·음·총획, 849개 한자어,
99자의 획순 좌표, 조각 조합)는 **아이가 아니라 어른에게도 쓸모가 있다** —
검정시험을 준비하거나, 자녀 학습 자료를 찾거나, 한자어 뜻을 확인하는 사람들.

그래서 같은 데이터로 **얼굴이 다른 사이트**를 하나 더 냈다. 게임의 하늘 그라디언트와
카툰 캐릭터를 쓰지 않고, 명조로 짠 조판에 인장 주홍 하나만 쓴다. 도메인도 나눴다 —
같은 도메인 안에 두 얼굴을 두면 어느 쪽 독자에게도 어정쩡해진다.

|                                                              |                                                                      |
| ------------------------------------------------------------ | -------------------------------------------------------------------- |
| [급수표](https://hanjasajeon.pages.dev/hanja/급수)           | 8급~4급 급수별 신규·누적 자수. 7급 시험 범위는 50자가 아니라 150자다 |
| [글자 1,028장](https://hanjasajeon.pages.dev/hanja/급수/8급) | 훈·음·총획·급수, 쓰이는 낱말, 글자를 이루는 조각, 획순               |
| [낱말 849장](https://hanjasajeon.pages.dev/hanja/낱말)       | 두 글자 한자어. 소리가 같고 한자가 다른 낱말도 갈라 준다             |
| 따라쓰기                                                     | 급수마다 인쇄용 활동지. 브라우저 인쇄 창에서 PDF 로 저장된다         |
| 퀴즈                                                         | 급수마다 훈·음 맞히기. 가입 없이, 풀 때마다 새 문제                  |

급수는 **사단법인 한국어문회가 배포하는 배정한자 자료를 그대로 따랐다.**
처음에는 시드 파일(게임의 마을 순서)을 급수로 쓰고 있었는데, 공식 자료와 대조하니
1,000자 중 133자가 틀려 있었다. 마을과 급수는 다른 축이라 한 값으로 둘 다
시키면 어느 한쪽이 반드시 틀어진다. 지금은 갈라 두었다.
...
| 낱말        | **849개** — 1028자 중 987자를 덮는다     |
| 캐릭터      | **6직업** · 레벨 10마다 전직             |
| 사전 페이지 | **1,907장** — 로그인 없이 열린다         |
...
| `npm run audit:seo`         | 구운 사전 1,907장을 전수 검사 (제목·설명·canonical 겹침) |
| `npm run indexnow`          | 사전 주소를 네이버·Bing 에 통보 (`-- --dry` 로 미리보기) |
